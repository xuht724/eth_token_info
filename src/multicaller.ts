...
//@ts-ignore
import balancerVault_abi_json from './abi/json/balancerVault.json'assert { type: "json" };
import { balancer_vault_address } from "./config";
...
export class MulticallHelper {
    ...
    constructor(node_url: string) {
        ...
        this.ethcallProvider = new Provider(1, this.provider);
    }
    ...
    public async batchGetBalancerEdgeBlockTimestamp(BalancerEdgeList: string[]): Promise<Map<string, number>> {
        const batchSize = 2000;
        let begin = performance.now();

        // Initialize the map to store results
        const resultMap: Map<string, number> = new Map();
        const BalancerVaultContract = new Contract(balancer_vault_address, balancerVault_abi_json);
        for (let i = 0; i < BalancerEdgeList.length; i += batchSize) {
            const batch = BalancerEdgeList.slice(i, i + batchSize);

            let ContractCalllist: Call[] = [];
            for (const BalancerEdge of batch) {
                ContractCalllist.push(BalancerVaultContract.getPoolTokens(BalancerEdge));
            }

            try {
                const poolTokens = await this.ethcallProvider.tryAll(ContractCalllist);

                let BalancerEdgeList: string[] = []
                let getBlockCallList: Promise<ethers.Block | null>[] = []
                poolTokens.forEach((result: any, index) => {
                    const balancerEdge = batch[index];
                    // Check if the result is not null
                    if (result !== null && result !== undefined) {
                        BalancerEdgeList.push(balancerEdge)
                        getBlockCallList.push(this.provider.getBlock(result.last))
                    }
                })
                const results = await Promise.all(getBlockCallList)
                results.forEach((result: any, index) => {
                    const balancerEdge = BalancerEdgeList[index];

                    // Check if the result is not null
                    if (result !== null && result !== undefined) {
                        const blockTimestamp = Number(result.timestamp); // Adjust this based on actual return data
                        resultMap.set(balancerEdge, blockTimestamp);
                    }
                })
            } catch (error) {
                // console.error('Error in batchGetV3EdgeBlockTimestamp:',message);
