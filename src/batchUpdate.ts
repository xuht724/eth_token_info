...
async function main() {
    ...
    } else if (edgeType === 'balancer') {
        if (level === -1) {
            // Fetch exceptional addresses for Balancer
            addressList = await sqliteHelper.getBalancerAddressesWithExceptions();
        } else {
            // Fetch regular addresses for Balancer
            addressList = await sqliteHelper.getBalancerEdgeAddresses(level);
        }
        getTimestampFunction = multicallHelper.batchGetBalancerEdgeBlockTimestamp.bind(multicallHelper);
        updateTimestampFunction = sqliteHelper.batchUpdateBalancerEdgeBlockTimestamp.bind(sqliteHelper);
    } else {
        console.error("Invalid edge type. Supported values are 'v2' or 'v3' or 'balancer'.");
        process.exit(1);
    }
