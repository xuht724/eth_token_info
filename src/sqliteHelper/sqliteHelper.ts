...
import { TokenInfo, v2Edge, v3Edge, ProtocolName, balancerEdge, BalancerPool } from '../types'; // Update the path to match your project structure
...
export class SqliteHelper {
    ...
    private createBalancerEdgeTable(): void {
        this.db.run(`
            CREATE TABLE IF NOT EXISTS BalancerEdge (
                address TEXT PRIMARY KEY,
                protocolName TEXT,
                poolType TEXT,
                pairId TEXT,
    ...
    public updateBalancerEdgeTime(address: string, newBlockTimeStamp: number): void {
        const tag = this.calculateTag(newBlockTimeStamp);
        const query = 'UPDATE BalancerEdge SET blockTimeLast = ?, tag = ? WHERE address = ?';
        const params = [newBlockTimeStamp, tag, address];

        this.db.run(query, params, (err) => {
            if (err) {
                this.logger.error('Error updating BalancerEdge:', err.message);
            } else {
                this.logger.info(`BalancerEdge with address ${address} updated.`);
            }
        });
    }
    ...
    public addBalancerEdge(edge: balancerEdge): void {
        ...
        this.db.run(query, params, (err) => {
            if (err) {
                this.logger.error(`Error adding BalancerEdge ${edge.pairAddress} : ${err.message}`);
            } else {
                this.logger.info(`BalancerEdge with address ${edge.pairAddress} added.`);
            }
        });
    }
    ...
    public async getBalancerAddressesWithExceptions(): Promise<string[]> {
        const query = 'SELECT pairId FROM BalancerEdge WHERE tag IS NULL OR (tag > 5)';
        try {
            const rows = await this.runQuery(query, []);

            const addresses: string[] = rows.map((row: any) => row.pairId);

            return addresses;
        } catch (error) {
            // this.logger.error('Error fetching Balancer addresses with exceptions:', error.message);
            throw error;
        }
    }
    ...
    public async getBalancerEdges(minTag: number = 0): Promise<balancerEdge[]> {
        const query = 'SELECT * FROM BalancerEdge WHERE tag >= ?';
        try {
            const rows = await this.runQuery(query, [minTag]);

            const BalancerEdges: balancerEdge[] = rows.map((row: any) => ({
                pairAddress: row.address,
                protocolName: row.protocolName,
            ...
            return BalancerEdges;
        } catch (error) {
            throw error;
        }
    }

    public async getBalancerEdgeAddresses(minTag: number = 0): Promise<string[]> {
    ...
    public async batchUpdateBalancerEdgeBlockTimestamp(dataMap: Map<string, number>): Promise<void> {
        const promises: Promise<void>[] = [];

        for (const [pairId, blockTimestampLast] of dataMap.entries()) {
            const tag = this.calculateTag(blockTimestampLast);

            const updateQuery =
                'UPDATE BalancerEdge SET blockTimeLast = ?, tag = ? WHERE pairId = ?';

            const queryParams = [blockTimestampLast, tag, pairId];
            console.log(queryParams);

            const promise = new Promise<void>((resolve, reject) => {
                this.db.run(updateQuery, queryParams, (err) => {
                    if (err) {
                        this.logger.error(`Error updating BalancerEdge for pairId ${pairId}:`, err.message);
                        reject(err);
                    } else {
                        this.logger.info(`BalancerEdge update complete for pairId ${pairId}.`);
                        resolve();
                    }
                });
            });
